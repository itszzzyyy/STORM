...
class ModelProcessor():
    ...
    def model_evaluate(self, model, all_q, all_gt, ks, use_mean_rank,
        ...
        # 查询轨迹的 lat, lon. tem
        # print("all_q",all_q)
        q_lat = np.array([x[2] for x in all_q],dtype=object)
        # print("q_lat",q_lat)
        q_lon = np.array([x[3] for x in all_q], dtype=object)
        q_tem = np.array([x[4] for x in all_q], dtype=object)
        q_array = np.array([x[1] for x in all_q],dtype=object)

        q_ids = [x[0] for x in all_q]
        print("q_ids: ", q_ids)
        gt_dedup = np.array(gt_dedup,dtype=object)
        # ground truth 的 lat, lon, tem
        gt_lat = np.array(gt_lat,dtype=object)
        gt_lon = np.array(gt_lon,dtype=object)
        gt_tem = np.array(gt_tem,dtype=object)
        ...
        q_lat = self.__pad_jagged_array(q_lat, max_len)
        q_lon = self.__pad_jagged_array(q_lon, max_len)
        q_tem = self.__pad_jagged_array(q_tem, max_len)
        gt_lat = self.__pad_jagged_array(gt_lat, max_len)
        gt_lon = self.__pad_jagged_array(gt_lon, max_len)
        gt_tem = self.__pad_jagged_array(gt_tem, max_len)
        print("processed_q_lat",q_lat[0])
        q_spa = np.stack((q_lat, q_lon), axis=-1)
        gt_spa = np.stack((gt_lat, gt_lon), axis=-1)
        # print("qpairs",q_lat_lon_np[0])
        # Generate the feature vector representation for the ground truth 
        if predict_batch_size == 0:
            print("Predicting GT traj")
            prediction_gt = model.predict([gt_dedup,gt_spa,gt_tem], verbose = 1)
        else:
            ...
            while True:
                ...
                gt_spa_batch = gt_spa[start_id:end_id]
                gt_spa_batch = np.squeeze(gt_spa_batch, axis=2)

                # 检查新的形状
                print("gtspabatch",gt_spa_batch.shape)
                gt_tem_batch = gt_tem[start_id:end_id]
                pred_batch = model.predict([gt_batch,gt_spa_batch,gt_tem_batch])
        ...
        q_spa = np.squeeze(q_spa,axis=2)
        print("q_spa", q_spa.shape)
        all_pred_q = model.predict([q_array,q_spa,q_tem])
        all_pred_q = all_pred_q.reshape((all_pred_q.shape[0],
                                         all_pred_q.shape[1]*all_pred_q.shape[2]))
        # print("all_pred_q", all_pred_q)
        ...
        for i in range(len(q_array)):
            ...
            if use_mean_rank:
                ...
                if rank >=20:
                    cnt +=1
                    wrong_traj.append([rank,q_array[i]])
                    # continue
                dbs=q_knn[:10]
                db_id.append(dbs)
                # print("db_id",dbs)
                ...
                # if rank >= 5:
                #     print("第几条轨迹",i)
                #     wrong_traj.append([rank,all_gt[i]])
        ...
        # print("异常轨迹数目",wrong_traj)
        saved_pair = []
        for i in range(0,len(q_ids)):
            saved_pair.append([q_ids[i],all_rank[i],db_id[i].tolist()])
        # print("saved_pair",saved_pair)
        pair_to_save = np.array(saved_pair,dtype=object)
        np.save('mean_rank',pair_to_save)
        return [all_hit_rates, mean_rank, avg_eval_time]


    def load_model(self,model, model_path, triplet_margin):
        # """
        ...
        def triplet_loss(y_true, y_pred):
            ...
            const = K.constant(triplet_margin, dtype='float32')
        ...
        # model = models.load_model("./MODEL_OUT/mymodel")
        # model.summary()
