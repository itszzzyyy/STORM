...
class MHA:
    ...
    def __init__(self, num_heads=8,d_model=512, attn_drop=0.0, proj_drop=0.0,
        ...
        query_input = Input(shape=(None,512))
        key_input = Input(shape=(None,512))
        batch_temporal_mat = Input(shape=(None,None))
        query = self.query_layer(query_input)
        key = self.key_layer(key_input)
        value = self.value_layer(key_input)
        ...
        output = self.proj(output)  # (B, T_q, dim_out)
        output = self.proj_drop(output)
        self.model = Model(inputs = [query_input,key_input,batch_temporal_mat],outputs = output)
        self.model.summary()
...
class Decoder():
    ...
    def __init__(self, src_traj_len, src_feature_size, trg_traj_len,
        ...
        outputs = self.attention(query=embedded_src, value=embedded_trg)
