...
class TrajProcessor():
    ...
    def process_training_data(self, all_pairs):
        ...
        for one_pair in all_pairs:
            ...
            gt_lat = self.embedinginput(gt,'lat')
            gt_lon = self.embedinginput(gt,'lon')

            gt_t = self.embedinginput(gt,'time')
            gt = self.__keep_id_only(gt)
            q_lat = self.embedinginput(q, 'lat')
            q_lon = self.embedinginput(q, 'lon')
            qt = self.embedinginput(q, 'time')
            ...
            one_x = np.array([gt, q, gt_patt_s, gt_patt_t, q_lat,q_lon, qt,gt_lat,gt_lon,gt_t], dtype=object)
            ...
            # print("q_lon",q_lon)
            # print("qt",qt)
            # print("gt_patt_s",gt_patt_s)
            # print("gt_patt_t",gt_patt_t)
            all_x.append(one_x)
            all_y.append(one_y)
    ...
    def embedinginput(self, trajectory,modes):
        if modes == 'lat':
            # print('loc')
            trajectory_ = copy.deepcopy(trajectory)
            traj_array = np.array([np.array([x[1][0]]) for x in trajectory_])
        elif modes == 'lon':
            # print('loc')
            trajectory_ = copy.deepcopy(trajectory)
            traj_array = np.array([np.array([x[1][1]]) for x in trajectory_])
        elif modes == 'time':
            # print('time')
            trajectory_ = copy.deepcopy(trajectory)
            traj_array = np.array([np.array([x[1][2]]) for x in trajectory_])
        return traj_array
