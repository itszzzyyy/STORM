...
class TestFileProcessor():
    ...
    def process_data(self, num_data, dataset_mode, min_traj_len, max_traj_len,
        ...
        while len(list_traj_1) < num_data:
            ...
            if new_traj is not None:
                ...
                traj_1 = new_traj[0::2]
                traj_2 = new_traj[1::2]
                db = copy.deepcopy(traj_2)
                ori_1 = traj_1
                ori_2 = traj_2
                dis = [0.2,0.3,0.4,0.5]
                db_new = []
                for item in dis:
                    tmp = self.__distort_spatiotemporal_traj(db,item,0)
                    db_new.append(tmp)
                # print("db_new", db_new)
                if drop_rate > 0:   
                ...
                processed_db = []
                for traj in db_new:
                    traj = self.__grid_trajectory(copy.deepcopy(traj))
                    traj = self.__remove_non_hot_cells(traj)
                    processed_db.append(traj)
                ...
                if t1_len >= min_traj_len and t2_len >= min_traj_len:
                    [traj_1, traj_lat_1,traj_lon_1,traj_tem_1, raw_traj_1] = self.__split_id_and_traj(traj_1)
                    [traj_2,traj_lat_2,traj_lon_2,traj_tem_2, raw_traj_2] = self.__split_id_and_traj(traj_2)
                    # print("traj_lon1",traj_lon_1)
                    # print("traj_lat1",traj_lat_1)

                    for traj in processed_db:
                        [trajs, trajs_lat,trajs_lon,trajs_tem, raw_trajs] = self.__split_id_and_traj(traj)
                        final_db_traj.append(np.array([cnt,trajs, trajs_lat,trajs_lon,trajs_tem],dtype=object))
                        final_db_traj_raw.append(np.array([cnt, raw_trajs],dtype=object))
                        cnt += 1
                    list_traj_1.append(np.array([start_ID, traj_1,traj_lat_1,traj_lon_1,traj_tem_1],dtype=object))
                    list_traj_1_raw.append(np.array([start_ID, raw_traj_1],dtype=object))
                    list_traj_2.append(np.array([start_ID, traj_2,traj_lat_2,traj_lon_2,traj_tem_2],dtype=object))
        ...
        if num_data == 200:
            ...
            print("list_1",list_traj_1[0])
        print("final_traj",final_db_traj)
        return [np.array(list_traj_1), np.array(list_traj_2),
                np.array(list_traj_1_raw), np.array(list_traj_2_raw),
                np.array(final_db_traj),np.array(final_db_traj_raw)]
    ...
    def __process_csv_porto(self, line, min_trajectory_length,
        ...
        if (len(trajectory) <= max_trajectory_length and 
            ...
            for i in range(0, len(trajectory)):
                time = datetime.fromtimestamp(trajectory[i][2])
                start_second = (time.hour * 3600 +
                                time.minute * 60 + time.second)
                if start_second >= self.__SECONDS_IN_DAY:
                    start_second -= self.__SECONDS_IN_DAY
                minute = int(start_second / self.__SECONDS_IN_MINUTE)
                trajectory[i][2] = minute
            # start_dtime = datetime.fromtimestamp(int(reads[1]))
    ...
    def __split_id_and_traj(self, trajectory):
        ...
        traj_lat = np.array([np.array([x[2][0]]) for x in trajectory_])
        traj_lon = np.array([np.array([x[2][1]]) for x in trajectory_])
        traj_tem = np.array([np.array([x[2][2]]) for x in trajectory_])
        raw_traj = np.array([x[1] for x in trajectory_])
        return [traj_id, traj_lat,traj_lon,traj_tem,raw_traj]
